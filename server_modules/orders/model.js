...
//Written new
//Get order history
function getOrderHistory(email, cb) {
  //console.log("sdfghjk = ",orderLocation); //change given_namej to take name from auth 0
  console.log("youse emails is = ",email)
  
  
  connection.query(
    ` SELECT order_id,users.given_name as user_name, created_date, shipped
    ...
    LEFT OUTER JOIN users ON (users.id = orders.user_id) WHERE users.email=? GROUP BY o_id  ORDER BY created_date DESC`, [email],
    (err, results) => {
      if (err) {
        cb(err);
        return;
